...
import { setDiff, setHasDiff } from "matrix-react-sdk/src/utils/sets";
import { Action } from "matrix-react-sdk/src/dispatcher/actions";
import { arrayHasDiff, arrayHasOrderChange, filterBoolean } from "matrix-react-sdk/src/utils/arrays";
import { reorderLexicographically } from "matrix-react-sdk/src/utils/stringOrderField";
import { TAG_ORDER } from "matrix-react-sdk/src/components/views/rooms/RoomList";
...
} from "matrix-react-sdk/src/stores/spaces/flattenSpaceHierarchy";
...
export class SpaceStoreClass extends AsyncStoreWithClient<IState> {
    ...
    public setActiveSpace(space: SpaceKey, contextSwitch = true): void {
        ...
        if (space.startsWith("plugin.")) {
            // For plugin just continue the operation
            contextSwitch = false;
    ...
    protected async onReady(): Promise<void> {
        ...
        let lastSpaceId = window.localStorage.getItem(ACTIVE_SPACE_LS_KEY);
        const newSpaceId = this.getActiveSpaceFromUrl();
        if (newSpaceId){
            lastSpaceId = newSpaceId;
        }
        const valid =
            lastSpaceId &&
    ...
    private getActiveSpaceFromUrl(): string | null {
        const location = window.location.href;
        if (location.endsWith("/plugins/files")){
            return "plugin.files";
        }
        else if (location.endsWith("/plugins/reports")){
            return "plugin.reports"
        }
        else if (location.endsWith("/plugins/algology")){
            return "plugin.algology"
        }
        return null;
    }
