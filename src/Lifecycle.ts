...
import { MatrixClient, OidcTokenRefresher, SSOAction, createClient } from "matrix-js-sdk/src/matrix";
import { QueryDict } from "matrix-js-sdk/src/utils";
import { MINIMUM_MATRIX_VERSION } from "matrix-js-sdk/src/version-support";
import { ReactNode } from "react";

import { SSO_HOMESERVER_URL_KEY, SSO_IDP_ID_KEY, SSO_ID_SERVER_URL_KEY } from "matrix-react-sdk/src/BasePlatform";
...
export async function setLoggedIn(credentials: IMatrixClientCreds): Promise<MatrixClient> {
    ...
    const pickleKey =
        credentials.userId && credentials.deviceId
            ? await PlatformPeg.get()?.getPickleKey(credentials.userId, credentials.deviceId)
            : null;

    console.log({ pickleKey });
