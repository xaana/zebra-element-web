...
export default class MatrixChat extends React.PureComponent<IProps, IState> {
    ...
    private async setApiUrl(): Promise<void> {
        try {
            ...
            if (configData?.plugins.reports.collabServerWebsocketUrl) {
                await SettingsStore.setValue(
                    "collabServerWebsocketUrl",
                    null,
                    SettingLevel.DEVICE,
                    configData.plugins.reports.collabServerWebsocketUrl,
                );
            }
            if (configData?.plugins.reports.collabServerHttpUrl) {
                await SettingsStore.setValue(
                    "collabServerHttpUrl",
                    null,
                    SettingLevel.DEVICE,
                    configData.plugins.reports.collabServerHttpUrl,
                );
            }
    ...
    public showScreen(screen: string, params?: { [key: string]: any }): void {
        ...
        } else if (screen.indexOf("plugins/") === 0) {
            const pluginName = screen.substring(8);
            this.setState({ activePluginName: pluginName });
        } else {
    ...
    public render(): React.ReactNode {
        ...
        if (this.state.view === Views.LOADING) {
            ...
            if (this.state.ready && !isStoreError) {
