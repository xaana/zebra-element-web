...
type AudioStreamAction = {
  setStreamCompleted: (
    streamComplete: AudioStreamState['streamCompleted']
  ) => void
}

export const audioStreamState = create<AudioStreamState & AudioStreamAction>(
  set => ({
    streamCompleted: false,
    setStreamCompleted: (streamComplete: boolean) =>
      set(() => ({ streamCompleted: streamComplete }))
  })
)
...
export const stopState = create<StopState & StopAction>(set => ({
  stopDetected: false,
  setStopDetected: (stopDetected: boolean) =>
    set(() => ({ stopDetected: stopDetected }))
}))
...
export const voiceBotState = create<State & Action>(set => ({
  ...
  setVoiceBotEnabled: (newDialogOpen: boolean) =>
    set(() => ({ voiceBotEnabled: newDialogOpen }))
}))
    ...
    while (true) {
      ...
      if (done) {
        if (partialChunk.trim()) {
          try {
            const parsedChunk: { type: string; data: string } =
              JSON.parse(partialChunk)
  ...
  const startRecording = async () => {
    try {
      ...
      audioContext.audioWorklet
        .addModule('http://localhost:5500/audio-processor.js')
        .then(() => {
            console.log('silence detecter started')
          const microphone =
            mic.current && audioContext.createMediaStreamSource(mic.current)
      ...
      >
        ...
        <DialogStyle>
          ...
          >
            ...
            >
              <div
                className="zexa-h-full zexa-w-full zexa-bg-white dark:zexa-bg-black"
                ref={loaderRef}
              >
                <LoadingAnimation />
              </div>
