...
export const Visualizer = ({ status }: { status: string }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      ringCoverRef.current?.material instanceof THREE.Material &&
        ringCoverRef.current?.material.dispose()
  ...
  function render() {
    if (
      ...
      if (acceleration.current > 0.35) {
        ...
        spinnerRef.current.style.transform = `scale(${
          0.8 + 0.2 * progress
        }) translate(-50%, -50%)`
      } else {
        cameraRef.current.position.z = 70
  ...
  const animate = () => {
    ...
    if (meshRef.current)
      meshRef.current.rotation.x += rotateValue.current + acceleration.current
    render()
    rendererRef.current &&
      sceneRef.current &&
      cameraRef.current &&
  ...
  useEffect(() => {
    if (groupRef.current && cameraRef.current) {
      if (status === 'loading') {
        ...
        if (ringCoverRef.current?.material instanceof THREE.Material)
          ringCoverRef.current.material.opacity = 1 // Assuming this is the final opacity
  ...
  return (
    <>
      <div
        ref={wrapRef}
        id="wrap"
        className="zexa-z-[1] zexa-w-full zexa-h-full"
      >
        <div
          ref={spinnerRef}
          className="zexa-absolute zexa-top-1/2 zexa-left-1/2"
