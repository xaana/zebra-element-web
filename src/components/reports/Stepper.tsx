...
export const Stepper = ({
  ...
  return (
    <div className='max-w-[1024px] mx-auto pt-6'>
      <StepperStyle
        className={cn('stepper max-w-[52rem] mx-auto px-3 md:px-0', className)}
      >
        <ul className='stepper__list w-full flex flex-col sm:flex-row gap-x-4 sm:items-center sm:justify-between list-none gap-y-4'>
          {steps.map((step, index) => (
            <li
              className={cn(
                'stepper__list__item text-center relative flex sm:flex-col sm:flex-1 items-center gap-2',
                {
                  ...
                  'stepper__list__item--current text-accent':
                    activeStep?.id === index,
            ...
            >
              ...
              <div className='stepper__list__title text-sm font-semibold'>
                {step.text}
              </div>
            </li>
          ))}
      ...
      <Progress
        value={(((activeStep?.id || 0) + 1) / steps.length) * 100}
        className='h-[2px] my-8'
      />
    </div>
  )
}
