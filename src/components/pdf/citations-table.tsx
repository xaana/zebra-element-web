...
export function CitationsTable({
  ...
  const columns: ColumnDef<Citation>[] = [
    {
      ...
      header: 'Query',
      cell: ({ row }) => (
        <div className="zexa-font-normal zexa-text-xs zexa-capitalize">
  ...
  if (new Set(data.map(citation => citation.doc_number)).size > 1) {
    columns.push({
      ...
      header: 'Document',
      cell: ({ row }) => (
        <div className="zexa-font-normal zexa-text-xs zexa-truncate">
  ...
  return (
    <>
      <Table>
          ...
          {table.getHeaderGroups().map(headerGroup => (
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map(header => {
                return (
                  <TableHead className="zexa-font-normal" key={header.id}>
        ...
        <TableBody className="zexa-border-b">
          {table.getRowModel().rows?.length ? (
            table.getRowModel().rows.map(row => (
              <TableRow
                key={row.id}
                className="zexa-cursor-pointer"
                onClick={async () =>
            ...
            <TableRow>
              <TableCell
                ...
                className="zexa-h-24 zexa-text-center"
              >
                No results.
              </TableCell>
            </TableRow>
